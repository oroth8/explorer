...
export default function CharacterCreator() {   
  ...
  function inputHandler(e){
    ...
    if(name==="age"){
      ...
      let charObj={age:value, birthYear:birth, currentYear:currentYear, userId:authContext.user._id}
      dispatch({type:"UPDATE_CHARACTER", char:charObj})
    }
    else {
      let charObj={name:value, currentYear:currentYear, userId:authContext.user._id}
      dispatch({type:"UPDATE_CHARACTER", char:charObj})
    }
  }
  function submitCharacter(e){
    ...
    if(e.target.name.value && e.target.age.value){
      ...
      let charObj={name:e.target.name.value, age:e.target.age.value, birthYear:birth, currentYear:currentYear, userId:authContext.user._id}
      ...
      <div className="row creation-box">
          <div className="col col-lg-4">
            <form onSubmit={submitCharacter}>
              <p><label htmlFor="name">Character Name:</label></p><input onChange={inputHandler} type="text" name="name" id="name" defaultValue={state.name}/>
              <p><label htmlFor="age">Age:</label></p><input type="number" onChange={inputHandler} name="age" maxLength="3" size="3" id="age" defaultValue={state.age}/>
